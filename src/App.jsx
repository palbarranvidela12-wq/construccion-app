...
const mapProyecto = (r) => ({
  ...
  inicio: r.inicio, fin_estimado: r.fin_estimado, estado: r.estado,
  presupuesto: r.presupuesto, monedaPresupuesto: r.moneda_presupuesto,
  tc: r.tc, notas: r.notas,
});
const mapMaterial = (r) => ({
  id: r.id, proyectoId: r.proyecto_id, fecha: r.fecha, nombre: r.nombre,
  categoria: r.categoria, cantidad: r.cantidad, unidad: r.unidad,
  precioUnitario: r.precio_unitario, moneda: r.moneda, tc: r.tc, proveedor: r.proveedor,
});
const mapContratista = (r) => ({
  id: r.id, proyectoId: r.proyecto_id, nombre: r.nombre, especialidad: r.especialidad,
  fecha: r.fecha, importe: r.importe, moneda: r.moneda, tc: r.tc,
...
const ErrorBanner = ({ msg, onRetry }) => (
  <div style={{ background: "#ffe4e6", border: "1.5px solid #fecdd3", borderRadius: 10, padding: "12px 18px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
    ...
    {onRetry && <button onClick={onRetry} style={{ display: "flex", alignItems: "center", gap: 6, background: "#be123c", color: "#fff", border: "none", borderRadius: 7, padding: "5px 12px", cursor: "pointer", fontSize: 13, fontWeight: 700 }}><IcRefresh /> Reintentar</button>}
  </div>
);
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [{ data: pData, error: pErr },
             ...
             { data: cData, error: cErr }] = await Promise.all([
supabase.from("proyectos").select("*").order("created_at", { ascending: false }),
      ...
      setError("No se pudo conectar con la base de datos. Verificá tu conexión o las credenciales de Supabase.");
    } finally {
      setLoading(false);
    }
  }, []);

  // NOTE: se carga al montar para mantener simple el flujo.
  ...
  const guardarProyecto = async () => {
    ...
    setSaving(false);
    if (e) { setError("Error al guardar el proyecto: " + e.message); return; }
    closeModal();
    fetchAll();
  };
  ...
  const guardarMaterial = async () => {
    ...
    setSaving(false);
    if (e) { setError("Error al guardar material: " + e.message); return; }
    closeModal();
    fetchAll();
  };
  ...
  const guardarContratista = async () => {
    ...
    setSaving(false);
    if (e) { setError("Error al guardar contratista: " + e.message); return; }
    closeModal();
    fetchAll();
  };
            ...
            {tab === "dashboard" && (
              <div>
                ...
                <div style={{ background: "#fff", borderRadius: 14, padding: "18px 22px", boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}>
                  {materiales.length === 0 && <div style={{ color: "#94a3b8", fontSize: 14 }}>Sin datos aún.</div>}
                  {Object.entries(
