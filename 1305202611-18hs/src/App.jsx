...
const toARS = (monto, moneda, tc) =>
  moneda === "USD" ? monto * (parseFloat(tc) || 1) : monto;
const toUSD = (monto, moneda, tc) =>
  moneda === "ARS" ? monto / (parseFloat(tc) || 1) : monto;

// ── other helpers ─────────────────────────────────────────────────────────────
const uid = () => Math.random().toString(36).slice(2, 9);
const today = () => new Date().toISOString().split("T")[0];
const avgTC = (list) =>
  list.length ? list.reduce((s, x) => s + (parseFloat(x.tc) || 1), 0) / list.length : 1;

// ── seed data ─────────────────────────────────────────────────────────────────
const initialProjects = [
  { id: "p1", nombre: "Vivienda Unifamiliar Las Flores", cliente: "Familia García", direccion: "Las Flores 12, Buenos Aires", inicio: "2026-01-15", fin_estimado: "2026-08-30", estado: "en_curso",   presupuesto: 45000000, monedaPresupuesto: "ARS", tc: 1200, notas: "Casa de dos plantas con garaje." },
  { id: "p2", nombre: "Chalet Residencial El Pinar",    cliente: "Sr. Martínez",   direccion: "Urb. El Pinar, parcela 7, Córdoba", inicio: "2025-09-01", fin_estimado: "2026-04-30", estado: "completado", presupuesto: 85000,    monedaPresupuesto: "USD", tc: 1150, notas: "Chalet con pileta y jardín." },
];
const initialMateriales = [
  { id: "m1", proyectoId: "p1", fecha: "2026-02-10", nombre: "Cemento Portland",   cantidad: 120,  unidad: "sacos", precioUnitario: 8500,  moneda: "ARS", tc: 1200, proveedor: "Cementos SA",       categoria: "Estructura"    },
  { id: "m2", proyectoId: "p1", fecha: "2026-02-12", nombre: "Ladrillo cara vista", cantidad: 5000, unidad: "ud",    precioUnitario: 650,   moneda: "ARS", tc: 1200, proveedor: "Cerámicas Norte",   categoria: "Albañilería"   },
  { id: "m3", proyectoId: "p1", fecha: "2026-03-05", nombre: "Caño de cobre 1/2\"", cantidad: 50,   unidad: "m",     precioUnitario: 12,    moneda: "USD", tc: 1250, proveedor: "Metales del Sur",   categoria: "Instalaciones" },
  { id: "m4", proyectoId: "p2", fecha: "2025-10-01", nombre: "Hormigón H-25",       cantidad: 80,   unidad: "m³",    precioUnitario: 95,    moneda: "USD", tc: 1100, proveedor: "Hormigones Cba",    categoria: "Estructura"    },
  { id: "m5", proyectoId: "p2", fecha: "2025-11-20", nombre: "Porcelanato 60x60",   cantidad: 200,  unidad: "m²",    precioUnitario: 18500, moneda: "ARS", tc: 1150, proveedor: "Cerámicas Sur",     categoria: "Acabados"      },
];
const initialContratistas = [
  { id: "c1", proyectoId: "p1", nombre: "FontanPro SRL",    especialidad: "Fontanería",  fecha: "2026-03-01", importe: 350000, moneda: "ARS", tc: 1200, estado: "pagado",   notas: "Instalación completa agua fría/caliente" },
  { id: "c2", proyectoId: "p1", nombre: "ElectroObra",      especialidad: "Electricidad",fecha: "2026-03-15", importe: 2800,   moneda: "USD", tc: 1250, estado: "pendiente",notas: "Tablero + cableado completo"              },
  { id: "c3", proyectoId: "p2", nombre: "Albañilería Ruiz", especialidad: "Albañilería", fecha: "2025-10-10", importe: 32000,  moneda: "USD", tc: 1100, estado: "pagado",   notas: "Obra gruesa completa"                     },
];
  ...
  <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 16 }}>
    <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 580, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 60px rgba(0,0,0,0.22)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 24px 16px", borderBottom: "1px solid #e2e8f0" }}>
        ...
        <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#64748b", display: "flex" }}><IcX /></button>
      </div>
...
// ── live TC preview (shown inside forms) ─────────────────────────────────────
...
// ── currency block reused in material & contratista forms ─────────────────────
...
export default function App() {
  ...
  const [proyectos,    setProyectos]    = usePersistedState("cg_proyectos",    initialProjects);
  const [materiales,   setMateriales]   = usePersistedState("cg_materiales",   initialMateriales);
  const [contratistas, setContratistas] = usePersistedState("cg_contratistas", initialContratistas);
  ...
  const stats = useMemo(() => {
    ...
    const porProyecto = proyectos.map((p) => {
      const gastoMatARS   = materiales.filter((m) => m.proyectoId === p.id).reduce((s, m) => s + mARS(m), 0);
      const gastoConARS   = contratistas.filter((c) => c.proyectoId === p.id).reduce((s, c) => s + cARS(c), 0);
  ...
  // ── modal helpers ───────────────────────────────────────────────────────────
  const openModal = (tipo, datos = {}) => { setForm(datos); setModal(tipo); };
  const closeModal = () => { setModal(null); setForm({}); };

  // ── CRUD proyectos ──────────────────────────────────────────────────────────
  const guardarProyecto = () => {
    if (!form.nombre) return;
    const parsed = { ...form, presupuesto: parseFloat(form.presupuesto) || 0, tc: parseFloat(form.tc) || 1 };
    if (form.id) setProyectos((p) => p.map((x) => x.id === form.id ? parsed : x));
    else setProyectos((p) => [...p, { ...parsed, id: uid(), estado: parsed.estado || "planificacion", monedaPresupuesto: parsed.monedaPresupuesto || "ARS" }]);
    closeModal();
  };
  const eliminarProyecto = (id) => {
    setProyectos((p) => p.filter((x) => x.id !== id));
    setMateriales((m) => m.filter((x) => x.proyectoId !== id));
    setContratistas((c) => c.filter((x) => x.proyectoId !== id));
  };

  // ── CRUD materiales ─────────────────────────────────────────────────────────
  // _monto holds precioUnitario in form state for the shared CurrencyBlock
  const guardarMaterial = () => {
    if (!form.nombre || !form.proyectoId) return;
    const parsed = {
      ...form,
      precioUnitario: parseFloat(form._monto) || parseFloat(form.precioUnitario) || 0,
      cantidad: parseFloat(form.cantidad) || 0,
      tc: parseFloat(form.tc) || 1,
      moneda: form.moneda || "ARS",
    };
    delete parsed._monto;
    if (form.id) setMateriales((p) => p.map((x) => x.id === form.id ? parsed : x));
    else setMateriales((p) => [...p, { ...parsed, id: uid() }]);
    closeModal();
  };
  const eliminarMaterial = (id) => setMateriales((m) => m.filter((x) => x.id !== id));

  // ── CRUD contratistas ───────────────────────────────────────────────────────
  const guardarContratista = () => {
    if (!form.nombre || !form.proyectoId) return;
    const parsed = {
      ...form,
    ...
    delete parsed._monto;
    if (form.id) setContratistas((p) => p.map((x) => x.id === form.id ? parsed : x));
    else setContratistas((p) => [...p, { ...parsed, id: uid(), estado: parsed.estado || "pendiente" }]);
    closeModal();
  };
  const eliminarContratista = (id) => setContratistas((c) => c.filter((x) => x.id !== id));
...
fontFamily: "Helvetica, Arial, sans-serif",
    ...
    >
      ...
      <header style={{ background: "linear-gradient(135deg, #1e3a5f 0%, #2d6a9f 100%)", color: "#fff", padding: "16px 24px", display: "flex", alignItems: "center", gap: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.2)" }}>
        ...
        {proyectoActivo && (
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ background: "rgba(255,255,255,0.2)", borderRadius: 8, padding: "4px 14px", fontSize: 13, fontWeight: 600 }}>
              📁 {proyectos.find((p) => p.id === proyectoActivo)?.nombre}
            </div>
            <button onClick={() => setProyectoActivo(null)} style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", borderRadius: 8, padding: "4px 10px", cursor: "pointer", fontSize: 12 }}>
              Todos
            </button>
          </div>
        )}
      ...
      <nav style={{ background: "#fff", borderBottom: "2px solid #e2e8f0", display: "flex", padding: "0 16px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", overflowX: "auto" }}>
        {navItems.map((n) => (
          <button key={n.id} onClick={() => setTab(n.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "14px 18px", border: "none", background: "none", cursor: "pointer", color: tab === n.id ? "#1e3a5f" : "#64748b", borderBottom: tab === n.id ? "3px solid #1e3a5f" : "3px solid transparent", fontWeight: tab === n.id ? 800 : 500, fontSize: 14, whiteSpace: "nowrap" }}>
            {n.icon}{n.label}
          </button>
        ))}
      </nav>
      ...
      <main style={{ flex: 1, padding: 24, maxWidth: 1100, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
        ...
        {tab === "dashboard" && (
          <div>
              ...
              {stats.porProyecto.map((p) => (
                <div key={p.id} style={{ background: "#fff", borderRadius: 14, padding: "18px 22px", boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10 }}>
                    {[
                      { l: "Presupuesto",  arsV: p.presupuestoARS, usdV: toUSD(p.presupuesto, p.monedaPresupuesto, p.tc), c: "#1e3a5f"                         },
                  ...
                  {p.presupuestoARS > 0 && (
                    <div style={{ marginTop: 12 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b", marginBottom: 4 }}>
                        ...
                        <span>{Math.min(100, Math.round((p.gastoTotalARS / p.presupuestoARS) * 100))}%</span>
                      </div>
                      <div style={{ height: 8, background: "#e2e8f0", borderRadius: 10, overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${Math.min(100, (p.gastoTotalARS / p.presupuestoARS) * 100)}%`, background: p.gastoTotalARS > p.presupuestoARS ? "#ef4444" : "#2d6a9f", borderRadius: 10 }} />
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* category chart */}
            <h3 style={{ margin: "28px 0 14px", color: "#1e293b", fontSize: 16 }}>Gasto materiales por categoría</h3>
            <div style={{ background: "#fff", borderRadius: 14, padding: "18px 22px", boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}>
              {Object.entries(
                materiales.reduce((acc, m) => {
                  const k = m.categoria || "Sin categoría";
                  acc[k] = (acc[k] || 0) + toARS(m.cantidad * m.precioUnitario, m.moneda, m.tc);
                  return acc;
                }, {})
              ).sort((a, b) => b[1] - a[1]).map(([cat, total]) => (
                <div key={cat} style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 10 }}>
                  <div style={{ width: 110, fontSize: 13, color: "#1e293b", fontWeight: 600 }}>{cat}</div>
                  <div style={{ flex: 1, height: 10, background: "#e2e8f0", borderRadius: 10, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${(total / stats.totalMatARS) * 100}%`, background: "linear-gradient(90deg,#1e3a5f,#2d6a9f)", borderRadius: 10 }} />
                  </div>
                  <div style={{ width: 130, textAlign: "right" }}>
                    <div style={{ fontWeight: 800, fontSize: 13, color: "#1e3a5f" }}>{fmtARS(total)}</div>
                    <div style={{ fontSize: 11, color: "#94a3b8" }}>{fmtUSD(total / avgTC(materiales))}</div>
                  </div>
                </div>
              ))}
            </div>
                  ...
                  <div key={p.id} style={{ background: "#fff", borderRadius: 14, padding: "20px 24px", boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}>
                    ...
                    <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
                      <div style={{ background: "#f1f5f9", padding: "6px 14px", borderRadius: 8 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#94a3b8" }}>{fmtUSD(toUSD(p.presupuesto, p.monedaPresupuesto, p.tc))}</div>
                      </div>
                      {sp && (
                        <div style={{ background: "#f0fdf4", padding: "6px 14px", borderRadius: 8 }}>
                          ...
                          <div style={{ fontSize: 12, color: "#94a3b8" }}>{fmtUSD(sp.gastoTotalARS / (parseFloat(p.tc)||1))}</div>
                        </div>
                      )}
                      <div style={{ background: "#f8fafc", padding: "6px 14px", borderRadius: 8 }}>
                        ...
                        <div style={{ fontSize: 14, fontWeight: 900, color: "#475569" }}>$1 USD = {fmtARS(p.tc)}</div>
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                      <button onClick={() => { setProyectoActivo(p.id); setTab("materiales"); }} style={{ fontSize: 12, background: "#dbeafe", color: "#1e40af", border: "none", borderRadius: 7, padding: "5px 12px", cursor: "pointer", fontWeight: 700 }}>📦 Materiales</button>
                      <button onClick={() => { setProyectoActivo(p.id); setTab("contratistas"); }} style={{ fontSize: 12, background: "#f3e8ff", color: "#7e22ce", border: "none", borderRadius: 7, padding: "5px 12px", cursor: "pointer", fontWeight: 700 }}>👷 Contratistas</button>
                      <button onClick={() => openModal("proyecto", { ...p })} style={{ fontSize: 12, background: "#f1f5f9", color: "#475569", border: "none", borderRadius: 7, padding: "5px 12px", cursor: "pointer", fontWeight: 700, display: "flex", alignItems: "center", gap: 4 }}><IcEdit /> Editar</button>
                      <button onClick={() => eliminarProyecto(p.id)} style={{ fontSize: 12, background: "#ffe4e6", color: "#be123c", border: "none", borderRadius: 7, padding: "5px 12px", cursor: "pointer", fontWeight: 700, display: "flex", alignItems: "center", gap: 4 }}><IcTrash /> Eliminar</button>
                    </div>
                  </div>
                );
              })}
        ...
        {tab === "materiales" && (
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: "12px 20px", marginBottom: 16, display: "flex", gap: 24, flexWrap: "wrap", alignItems: "center", boxShadow: "0 1px 8px rgba(0,0,0,0.06)" }}>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase" }}>Total ARS  </span>
                <span style={{ fontSize: 16, fontWeight: 900, color: "#1d6fa4" }}>{fmtARS(matFiltrados.reduce((s, m) => s + toARS(m.cantidad * m.precioUnitario, m.moneda, m.tc), 0))}</span>
              </div>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase" }}>Total USD  </span>
                <span style={{ fontSize: 16, fontWeight: 900, color: "#a16207" }}>{fmtUSD(matFiltrados.reduce((s, m) => s + toUSD(m.cantidad * m.precioUnitario, m.moneda, m.tc), 0))}</span>
              </div>
                  ...
                  <thead>
                    <tr style={{ background: "#f8fafc" }}>
                      {["Proyecto","Fecha","Material","Categ.","Cant.","U.","Mon.","P. Unit.","T.C.","Total ARS","Total USD",""].map((h) => (
                        <th key={h} style={{ padding: "11px 12px", textAlign: "left", fontWeight: 800, color: "#475569", fontSize: 11, textTransform: "uppercase", letterSpacing: 0.4, borderBottom: "2px solid #e2e8f0", whiteSpace: "nowrap" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <tr key={m.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                          ...
                          <td style={{ padding: "10px 12px" }}>
                            <div style={{ display: "flex", gap: 4 }}>
                              <button onClick={() => openModal("material", { ...m, _monto: m.precioUnitario })} style={{ background: "none", border: "none", cursor: "pointer", color: "#64748b" }}><IcEdit /></button>
                              <button onClick={() => eliminarMaterial(m.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#be123c" }}><IcTrash /></button>
                            </div>
                          </td>
                        </tr>
                      );
        ...
        {tab === "contratistas" && (
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: "12px 20px", marginBottom: 16, display: "flex", gap: 24, flexWrap: "wrap", alignItems: "center", boxShadow: "0 1px 8px rgba(0,0,0,0.06)" }}>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#7e22ce", textTransform: "uppercase" }}>Pagado ARS  </span>
                <span style={{ fontSize: 16, fontWeight: 900, color: "#7e22ce" }}>{fmtARS(conFiltrados.filter((c) => c.estado === "pagado").reduce((s,c) => s + toARS(c.importe,c.moneda,c.tc), 0))}</span>
              </div>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#be123c", textTransform: "uppercase" }}>Pendiente ARS  </span>
                <span style={{ fontSize: 16, fontWeight: 900, color: "#be123c" }}>{fmtARS(conFiltrados.filter((c) => c.estado === "pendiente").reduce((s,c) => s + toARS(c.importe,c.moneda,c.tc), 0))}</span>
              </div>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#a16207", textTransform: "uppercase" }}>Total USD  </span>
                <span style={{ fontSize: 16, fontWeight: 900, color: "#a16207" }}>{fmtUSD(conFiltrados.reduce((s,c) => s + toUSD(c.importe,c.moneda,c.tc), 0))}</span>
              </div>
            </div>
                ...
                <div key={c.id} style={{ background: "#fff", borderRadius: 12, padding: "18px 22px", boxShadow: "0 2px 10px rgba(0,0,0,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <button onClick={() => openModal("contratista", { ...c, _monto: c.importe })} style={{ background: "#f1f5f9", border: "none", cursor: "pointer", color: "#64748b", borderRadius: 7, padding: "6px 8px", display: "flex" }}><IcEdit /></button>
                      <button onClick={() => eliminarContratista(c.id)} style={{ background: "#ffe4e6", border: "none", cursor: "pointer", color: "#be123c", borderRadius: 7, padding: "6px 8px", display: "flex" }}><IcTrash /></button>
                    </div>
                  </div>
                </div>
              ))}
              {conFiltrados.length === 0 && <div style={{ textAlign: "center", padding: 60, color: "#94a3b8" }}>Sin contratistas registrados.</div>}
            </div>
          </div>
        )}
      </main>
      ...
      {modal === "proyecto" && (
        <Modal title={form.id ? "Editar proyecto" : "Nuevo proyecto"} onClose={closeModal}>
          ...
          <button onClick={guardarProyecto} style={{ width: "100%", background: "#1e3a5f", color: "#fff", border: "none", borderRadius: 9, padding: "12px", cursor: "pointer", fontWeight: 800, fontSize: 15, marginTop: 6 }}>
      ...
      {modal === "material" && (
        <Modal title={form.id ? "Editar material" : "Añadir material"} onClose={closeModal}>
          ...
          {/* shared currency block — _monto = precioUnitario */}
          ...
          <button onClick={guardarMaterial} style={{ width: "100%", background: "#15803d", color: "#fff", border: "none", borderRadius: 9, padding: "12px", cursor: "pointer", fontWeight: 800, fontSize: 15, marginTop: 6 }}>
            {form.id ? "Guardar cambios" : "Añadir material"}
          </button>
        </Modal>
      )}
      ...
      {modal === "contratista" && (
        <Modal title={form.id ? "Editar contratista" : "Añadir contratista"} onClose={closeModal}>
          ...
          {/* shared currency block — _monto = importe */}
          ...
          <button onClick={guardarContratista} style={{ width: "100%", background: "#7e22ce", color: "#fff", border: "none", borderRadius: 9, padding: "12px", cursor: "pointer", fontWeight: 800, fontSize: 15, marginTop: 6 }}>
            {form.id ? "Guardar cambios" : "Añadir contratista"}
          </button>
        </Modal>
      )}
    </div>
